...
private[storage] trait ScanBehavior[K, C, S] extends StrictLogging { this: FSM[State, Data] with CompactBehavior[K, C, S] =>
  ...
  when(Scanning) {
    ...
    case Event(StorePlan(plan, promise), updates: UpdatedEntities) =>
      ...
      val originalUpdates =
        addAppVersions(
          plan.original.transitiveApps.toIterable.map(app => app.id -> app.version.toOffsetDateTime)(collection.breakOut),
          updates.appVersionsStored)
      val allUpdates =
        addAppVersions(plan.target.transitiveApps.toIterable.map(app => app.id -> app.version.toOffsetDateTime)(collection.breakOut), originalUpdates)
